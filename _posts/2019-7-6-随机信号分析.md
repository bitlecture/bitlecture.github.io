...
* 全概率公式
  $$
  P[A]=\sum_{j=1}^{N}{P\mid\|B_j]P[B_j]}
  $$

* 贝叶斯公式
  $$
  P[B\mid\|A]=\frac{P\mid\|B_i]P[B_i]}{\sum_{j=1}^{N}{P\mid\|B_j]P[B_j]}}
  $$

* 统计独立
  $$
  P\mid\|A]=P[B] or P[A\cap B]=P[A]P[B]
  $$
...
* 条件概率密度 $ f_\mid\|Y}\mid\|y)=\frac{f_{XY}(x,y)}{f_Y(y)} $
* 条件概率分布 $ F_\mid\|Y}\mid\|y)=\int_{-\infty}^{x}{f_\mid\|Y}(u,y)}du $
...
* **方法**：求某RV的数学期望（以离散型变量为例）
  ...
  3. $ E[Y]=\sum_{i=1}^{M}{E\mid\|X=x_i]P[X=x_i]} $（利用条件分布)
...
* 数学期望的**性质**
  ...
  * $E[X]=E[E\mid\|Y]]$
  * $E[g(Y\mid\|Y]=g(Y)E\mid\|Y]$
  * $E[g(\mid\|Y]=g(Y)$
  * $E[(X-E\mid\|Y])^2]\leq E[(X-g(Y))^2]$
...
* 平稳随机过程的**性质**
  ...
  7. 若X(t)为不包含任何周期分量的非周期WSS，则$ \lim\mid\|\t\mid\|\rightarrow \infty}R_X(\tau)=m_X^2  $
...
* $ R_{XY}(\tau) $的性质
  ...
  2. \mid\|R_{XY}(\ta\mid\|^2\leq R_X(0)R_Y(0)  $
  3. \mid\|R_{XY}(\ta\mid\|\leq\frac{1}{2}[R_X(0)+R_Y(0)] $
...
* $ S_{XY}(\omega),S_{YX}(\omega) $的性质：
  ...
  3. \mid\|S_{XY}(\omeg\mid\|^2\leq S_{X}(\omega)S_{Y}(\omega) $
  4. 其实部为偶函数，虚部为奇函数
...
* 解析过程的**性质**
  ...
  2. $ S_{\widehat X}(\omega\mid\|H(\omeg\mid\|^2S_X(\omega)=S_X(\omega) $
...
* 性质
  ...
  1. $ f_X(x_1,...,x_n)=f_X(x\mid\|x_{n-1})f_X(x_{n-\mid\|x_{n-2})...f_X(x_1) $

  2. 马尔可夫序列的子序列是马尔可夫序列。

  3. 马尔可夫序列的逆序列是马尔可夫序列。

  4. 条件均值$ E[X\mid\|X_{n-1},...,X_1]=E[X\mid\|X_{n-1}] $
